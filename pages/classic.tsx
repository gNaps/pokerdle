...
import { HomeProps, PokemonResult, Result } from "../models/models";
...
const Classic: NextPage<HomeProps> = ({ pokemon }) => {
  ...
  return (
    <Layout>
      <div className="h-screen bg-classic px-2 py-3">
        ...
        <div className="fixed bottom-0 shadow-2xl px-3 py-2 w-96 flex bg-white items-center justify-between">
          ...
          {!win && attempts >= 6 && (
            <>
              <p>You lose! Pokemon was {pokemon.name}! </p>
              <button
                className="bg-violet-500 px-3 py-3 rounded-md 
            hover:bg-violet-700 active:bg-violet-700 text-slate-50 ml-2"
                onClick={() => shareResult()}
